          ...
          child: ConstrainedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                _SuccessHero(r: r, controller: controller),
                ...
                _PassengerConfirmationCard(r: r, controller: controller),
                SizedBox(height: r.adaptive(phone: 14, smallPhone: 12, tablet: 16, desktop: 18)),
                _PaymentTimerCard(r: r, controller: controller),
...
class _SuccessHero extends StatelessWidget {
  const _SuccessHero({required this.r, required this.controller});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(
          'Durée réelle : ${controller.realDuration}',
          style: AppTextStyles.bodySmall(r).copyWith(
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              _SummaryItem2(r: r, icon: Icons.route_rounded, value: '${controller.distanceKm}km', label: 'Distance'),
              _SummaryItem2(r: r, icon: Icons.timer_rounded, value: controller.realDuration, label: 'Durée'),
              _SummaryItem2(r: r, icon: Icons.people_rounded, value: '${controller.passengersCount}', label: 'Passagers'),
            ],
          ),
          Divider(height: r.adaptive(phone: 20, smallPhone: 16, tablet: 24, desktop: 28), color: AppColors.border),
          _RevRow(r: r, label: 'Montant brut', value: '${controller.grossRevenue.toStringAsFixed(0)} FCFA', isBold: false),
          SizedBox(height: r.adaptive(phone: 4, smallPhone: 3, tablet: 5, desktop: 6)),
          _RevRow(r: r, label: 'Commission (10%)', value: '-${controller.commission.toStringAsFixed(0)} FCFA', isBold: false, valueColor: const Color(0xFFE53935)),
          Divider(height: r.adaptive(phone: 16, smallPhone: 12, tablet: 18, desktop: 20), color: AppColors.border),
          _RevRow(r: r, label: 'Vos revenus nets', value: '${controller.netRevenue.toStringAsFixed(0)} FCFA', isBold: true),
        ],
      ),
    );
  }
...
class _SummaryItem2 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Icon(icon, size: r.adaptive(phone: 22, smallPhone: 20, tablet: 24, desktop: 26), color: AppColors.primary),
        SizedBox(height: r.adaptive(phone: 4, smallPhone: 3, tablet: 5, desktop: 6)),
...
class _RevRow extends StatelessWidget {
  const _RevRow({required this.r, required this.label, required this.value, required this.isBold, this.valueColor});
  final AppResponsive r;
  final String label;
  final String value;
  final bool isBold;
    ...
    return Row(
      ...
      children: [
        Text(label, style: isBold
            ? AppTextStyles.bodyMedium(r).copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)
            : AppTextStyles.bodySmall(r).copyWith(color: AppColors.textMuted)),
        Text(value, style: isBold
            ? AppTextStyles.bodyMedium(r).copyWith(color: AppColors.primary, fontWeight: FontWeight.w800)
            : AppTextStyles.bodySmall(r).copyWith(color: valueColor ?? AppColors.textPrimary, fontWeight: FontWeight.w600)),
      ],
    );
  }
}
...
class _PassengerConfirmationCard extends StatelessWidget {
  const _PassengerConfirmationCard({required this.r, required this.controller});
  final AppResponsive r;
  final EndTripController controller;
            ...
            children: [
              Row(
                children: [
                  ...
                  const Spacer(),
                  Text(
                    '${controller.confirmedCount}/${controller.confirmations.length}',
                    style: AppTextStyles.bodySmall(r).copyWith(
                      color: controller.allConfirmed ? AppColors.primary : AppColors.warning,
              ...
              ...controller.confirmations.map((c) => Padding(
                    padding: EdgeInsets.only(bottom: r.adaptive(phone: 8, smallPhone: 6, tablet: 10, desktop: 12)),
                    child: Row(
                      children: [
                        Container(
...
class _PaymentTimerCard extends StatelessWidget {
  const _PaymentTimerCard({required this.r, required this.controller});
  final AppResponsive r;
  final EndTripController controller;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: r.adaptive(phone: 4, smallPhone: 3, tablet: 5, desktop: 6)),
                Text(
                  'Vos fonds seront disponibles le ${controller.availableDate}',
                  style: AppTextStyles.labelSmall(r).copyWith(
                    color: AppColors.textMuted,
                  ),
                ),
