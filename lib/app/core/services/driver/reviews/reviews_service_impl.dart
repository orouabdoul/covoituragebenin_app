...
class ReviewsServiceImpl implements ReviewsService {
  ...
  Future<ApiResult<ReviewsBodyModel>> fetchReviews({int page = 1}) async {
    try {
      ...
      logger.w('fetchReviews: ${e.type}');
      return ApiResult.failure(AppDio.classifyDioError(e));
    } catch (e) {
      logger.w('fetchReviews: $e');
      return ApiResult.failure(AppError.unexpected);
    }
  }
  ...
  Future<ApiResult<void>> replyToReview(String uuid, String reply) async {
    try {
      ...
      logger.w('replyToReview: ${e.type}');
      return ApiResult.failure(AppDio.classifyDioError(e));
    } catch (e) {
      logger.w('replyToReview: $e');
      return ApiResult.failure(AppError.unexpected);
    }
  }
  ...
  Future<ApiResult<Map<String, dynamic>>> reactToReview(
    ...
    try {
      ...
      logger.w('reactToReview: ${e.type}');
      return ApiResult.failure(AppDio.classifyDioError(e));
    } catch (e) {
      logger.w('reactToReview: $e');
      return ApiResult.failure(AppError.unexpected);
    }
  }
  ...
  Future<ApiResult<TripReviewsModel>> fetchTripReviews(String tripUuid) async {
    try {
      ...
      logger.w('fetchTripReviews: ${e.type}');
      return ApiResult.failure(AppDio.classifyDioError(e));
    } catch (e) {
      logger.w('fetchTripReviews: $e');
      return ApiResult.failure(AppError.unexpected);
    }
  }
}
